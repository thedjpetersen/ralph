...
import { useAITone } from '../stores/aiTone';
import { useSyncExternalStore } from 'react';
import './GhostTonePreview.css';

/**
 * GhostTonePreview displays an inline ghost text preview for AI tone adjustments.
 ...
 */
export function GhostTonePreview() {
  const { ghostPreviewInfo, targetElement, isLoading, previewText } = useAITone();
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      <span className="ghost-tone-before">{ghostPreviewInfo.beforeText}</span>
      <span className="ghost-tone-original">{ghostPreviewInfo.originalText}</span>
      <span className="ghost-tone-replacement">{ghostPreviewInfo.replacementText}</span>
      <span className="ghost-tone-after">{ghostPreviewInfo.afterText}</span>
    </div>
  );
}

GhostTonePreview.displayName = 'GhostTonePreview';

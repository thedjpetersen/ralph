...
func (_q *TransactionQuery) QueryReceipt() *ReceiptQuery {
...
func (_q *TransactionQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *TransactionQuery) FirstIDX(ctx context.Context) string {
...
func (_q *TransactionQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *TransactionQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *TransactionQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *TransactionQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *TransactionQuery) WithReceipt(opts ...func(*ReceiptQuery)) *TransactionQuery {
	query := (&ReceiptClient{config: _q.config}).Query()
...
//		GroupBy(transaction.FieldReceiptID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Transaction.Query().
//		Select(transaction.FieldReceiptID).
//		Scan(ctx, &v)
...
func (_q *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := _q.withReceipt; query != nil {
		if err := _q.loadReceipt(ctx, query, nodes, nil,
			func(n *Transaction, e *Receipt) { n.Edges.Receipt = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TransactionQuery) loadReceipt(ctx context.Context, query *ReceiptQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Receipt)) error {
...
func (_q *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(transaction.Table, transaction.Columns, sqlgraph.NewFieldSpec(transaction.FieldID, field.TypeString))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withReceipt != nil {
			_spec.Node.AddColumnOnce(transaction.FieldReceiptID)
		}
	}
